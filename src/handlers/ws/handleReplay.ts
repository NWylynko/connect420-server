...
export const handleReplay = async (
  ...
  try {
    ...
    if (socket.id === player1) {
        ...
        return replay ? `${socket.id} wants to replay` : `${socket.id} doesn't wants to replay`;
...
const checkForReplay = async (room: string): Promise<boolean> => {
  log.info("checking replay")
    let player1Replay = await redis.hgetAsync(gameHash(room), "player1Replay");
    let player2Replay = await redis.hgetAsync(gameHash(room), "player2Replay");
